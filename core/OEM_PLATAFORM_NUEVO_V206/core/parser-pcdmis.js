(function(){
  ...
  function rawAxis(r){
    ...
    if(c==='TP'||/\bTP\b|TRUE\s*POSITION|POSICION/.test(t))return'TP';
    if(c==='X'||/UBICACION\s*X|LOCATION\s*X/.test(b))return'X';
    if(c==='Y'||/UBICACION\s*Y|LOCATION\s*Y/.test(b))return'Y';
    if(c==='Z'||/UBICACION\s*Z|LOCATION\s*Z/.test(b))return'Z';
    if(c==='DF')return'DF';
    if(/^D\d*$/.test(c)||c==='D'||/UBICACION\s*D|LOCATION\s*D|DIAM/.test(b))return'D';
    if(c==='M'||/^DIST/.test(a)||/DIMENSION\s*M|DISTANCE/.test(t))return'M';
  ...
  window.parsePcDmisRows=function(rows){
    const meta={},rawBlocks={},itemsByCota={};let current=null;
    ...
    for(let i=0;i<rows.length;i++){
      ...
      if(Number.isFinite(num(ro.E_medida)))itemsByCota[current].push(itemObj(ro));
    }
    ...
    return {meta,measures,debug,rawBlocks};
  };
})();
