(function(){
  ...
  function rawAxis(r){
    ...
    // V218: nunca convertir X/Y/Z/D/2D/3D POSITION en TP por texto.
    // La columna C manda. Solo TP real si C=TP, nombre empieza por TP,
    // o el texto contiene TRUE POSITION literal.
    if(/^TP(\s|_|-|$)/.test(a)||/^TP(\s|_|-|$)/.test(b)||/TRUE\s*POSITION/.test(t))return'TP';
  ...
  window.parsePcDmisRows=function(rows){
    ...
    for(let i=0;i<rows.length;i++){
      ...
      // V212: SOLO una fila cabecera real puede abrir un bloque COTA.
      // Antes se buscaba COTA en toda la fila; eso rompía casos como:
      // UBIC1 | Ubicación de la dimensión (PLANO COTA 114) | Z | 114 | 113.989...
      // Esa fila NO es cabecera, es una medición de la COTA 1.
      const markerCell=norm(cell(r,0));
      const hasMeasurement=Number.isFinite(num(cell(r,3)))||Number.isFinite(num(cell(r,4)));
      const markerMatch=markerCell.match(/^\*+\s*COTA\s*(\d+)\s*\*+$/)||markerCell.match(/^COTA\s*[*#:\- ]*\s*(\d+)\s*$/);
      if(markerMatch&&!hasMeasurement){current=ensure(markerMatch[1]);rawBlocks[current].push(rawObj(current,r,i+1,true));continue;}
      if(!current)continue;
