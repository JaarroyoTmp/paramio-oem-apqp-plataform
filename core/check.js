...
const PLAN_VERSION='V230_NESTED_OPERATIONS';
...
function allRows(){const lib=loadLibrary().map(x=>Object.assign({source:'library'},x));const rows=[];BASE_ROWS.forEach(r=>{rows.push(r);if(r.band&&r.op==='020'){charRows().forEach(c=>rows.push(c));lib.filter(x=>String(x.op||'').padStart(3,'0')>='020'&&String(x.op||'').padStart(3,'0')<'030').forEach(x=>rows.push(x));}});lib.filter(x=>{const op=String(x.op||'').padStart(3,'0');return op<'020'||op>='030';}).forEach(x=>rows.push(x));return rows.sort((a,b)=>{if(a.band||b.band)return 0;return String(getVal(a,'op')||a.op).localeCompare(String(getVal(b,'op')||b.op),undefined,{numeric:true});});}
...
function renderTable(rows){let currentBand='';let html='';rows.forEach(r=>{if(r.band){currentBand=r.op;html+=`<tr class="opBand"><td colspan="14">OP ${esc(r.op)} · ${esc(r.title)}</td></tr>`;return;}if(r.subheader){html+=`<tr class="substepRow"><td></td><td colspan="13"><b>${esc(r.title)}</b></td></tr>`;return;}const src=r.source==='inspection'?'inspectionRow':(r.source==='library'?'libraryRow':(r.controlClass==='Subproceso'?(r.subClass==='firstoff'?'firstoffRow':'setupRow'):'baseRow'));html+=`<tr class="${src} ${r.kcs?'kcsRow':''}">
<td>${edit(r,'op','op')}</td><td>${edit(r,'process')}</td><td>${edit(r,'machine')}</td><td class="pcCenter">${esc(r.globo||'')}</td><td>${badge(r)}</td><td class="pcCenter"><b>${esc(r.featureType||'')}</b></td><td>${edit(r,'characteristic')}</td><td>${edit(r,'specification')}</td><td>${edit(r,'method')}</td><td>${edit(r,'equipment')}</td><td>${edit(r,'sample','sample')}</td><td>${edit(r,'frequency')}</td><td>${edit(r,'owner')}</td><td>${edit(r,'reaction')}</td></tr>`;});return html;}
function exportCsv(){const rows=allRows().filter(r=>!r.band);const headers=['OP','Proceso','Máquina/útiles','Globo','Clase','Tipo cota','Característica/control','Especificación','Método de control','Equipo','Muestra','Frecuencia','Responsable','Reacción'];const lines=[headers.join(';')];rows.forEach(r=>{lines.push([getVal(r,'op'),getVal(r,'process'),getVal(r,'machine'),r.globo||'',r.kcs?'KCS':getVal(r,'controlClass'),r.featureType||'',getVal(r,'characteristic'),getVal(r,'specification'),getVal(r,'method'),getVal(r,'equipment'),getVal(r,'sample'),getVal(r,'frequency'),getVal(r,'owner'),getVal(r,'reaction')].map(v=>'"'+String(v??'').replace(/"/g,'""')+'"').join(';'));});const blob=new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='plan_control_'+((project.meta||{}).reference||'referencia')+'_V230.csv';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500);}
...
<section class="cpTableWrap"><table class="cpTable"><thead><tr><th>Nº OP</th><th>Descripción operación</th><th>Máquina / útiles</th><th>Globo</th><th>Clasificación</th><th>Tipo cota</th><th>Característica producto / proceso</th><th>Especificación / tolerancia</th><th>Método de control</th><th>Instrumento de Evaluación / Medida</th><th>Muestra</th><th>Frecuencia</th><th>Responsable</th><th>Plan de reacción</th></tr></thead><tbody id="planBody">${renderTable(filteredRows())}</tbody></table></section><div class="cpFooterNote"><span>Documento generado desde Paramio Quality Documentation · ${PLAN_VERSION}</span><span>Las mediciones/resultados se gestionan en ISIR y Dimensional; el Plan de Control mantiene método, frecuencia, responsabilidad y reacción.</span></div>`;}
function renderTableOnly(){const body=document.getElementById('planBody');if(body)body.innerHTML=renderTable(filteredRows());}
render();
