(function(){
  ...
  function nominalFrom(li,ls,value,featureType){
    const explicit=firstNumber(value);
  ...
  window.parseInspectionWorkbook=function(wb){
    ...
    for(let i=hr+1;i<rows.length;i++){
      ...
      out.push({number:n,balloon:String(n),value,rawValue:rawCell,decodedValue:decoded,featureType,nominal:nominalFrom(lower,upper,decoded||value,featureType),units:fmt(r[cunit]),upper,lower,kcs:false,comment:'',assignedOp:'20',results:{}});
    }
