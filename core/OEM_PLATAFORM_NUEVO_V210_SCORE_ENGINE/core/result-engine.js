(function(){
  ...
  function rowTypeCompatible(c,it){
    ...
    if(ft==='POSITION')return ax==='TP'||/\bTP\b|TRUE\s*POSITION|DIMENSION\s+DE\s+POSICION/.test(t);
    if(ft==='PROFILE')return ax==='PROFILE'||/PROFILE|PERFIL/.test(t)||(/LOCATION|UBICACION/.test(t)&&['X','Y','Z'].includes(ax));
  ...
  function scoreCandidate(c,it){
    ...
    if(ft==='DIMENSION'){
      ...
      if(ft==='POSITION'&&ax==='TP')score+=40;
      if(ft==='PROFILE'&&/PROFILE|PERFIL/.test(t))score+=35;
      if(ft==='PROFILE'&&/LOCATION|UBICACION/.test(t))score+=12;
  ...
  window.allItems=function(c,piece){
    ...
    // V210: no bloquearse en el bloque numerico si hay una coincidencia semantica mucho mejor.
